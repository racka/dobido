...
public class ColorFadeAnimation extends SiteAnimation {
    ...
    public ColorFadeAnimation(AbsolutePanel container, Widget target, SiteCallback callback, int duration,
                              String targetColor) {
        super(container, target, callback, duration);

        widgetStyle = target.getElement().getStyle();
        this.targetColor = new RGB(targetColor);
        this.startColor = new RGB(widgetStyle.getColor());
    }
    ...
    public void animationLogic(double progress) {
        ...
        widgetStyle.setColor(new RGB(actualRed, actualGreen, actualBlue).toString());
    }
